...
opt = Optim.Options(g_tol=big(1e-22),
                x_tol=big(1e-22),
                f_tol=big(1e-22),
                time_limit=big(3600.0), # half hour
                show_trace = true,
...
tpl = (2, n_gammas)
fE = (x,y)->convert(Float64,MGCLaguerre.laguerre_density([convert(BigFloat,x), convert(BigFloat,y)], E))
g = (x,y)->convert(Float64,MGCLaguerre.laguerre_density([convert(BigFloat,x), convert(BigFloat,y)], coefs))
...
Plots.savefig(p,"MLNTest_$m.png")
